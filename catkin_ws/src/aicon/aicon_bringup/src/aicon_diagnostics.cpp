...
#include <aicon_msgs/SensorState.h>
#include <aicon_msgs/VersionInfo.h>
#include <string>
...
ros::Publisher aicon_d_version_info_pub;
ros::Publisher aicon_d_diagnostics_pub;
...
void sensorStateMsgCallback(const aicon_msgs::SensorState::ConstPtr &msg)
{
  if (msg->battery > 11.0)
  ...
  if (msg->button == aicon_msgs::SensorState::BUTTON0)
    setButtonDiagnosis(diagnostic_msgs::DiagnosticStatus::OK, "BUTTON 0 IS PUSHED");
  else if (msg->button == aicon_msgs::SensorState::BUTTON1)
...
void firmwareVersionMsgCallback(const aicon_msgs::VersionInfo::ConstPtr &msg)
{
  ...
  if (check_version == false)
  {
    if (firmware_version.major_number == FIRMWARE_VERSION_MAJOR_NUMBER)
    {
      if (firmware_version.minor_number > FIRMWARE_VERSION_MINOR_NUMBER)
      {
        ...
        ROS_WARN("You can find how to update its");
      }
    }
    else
    {
  ...
  aicon_msgs::VersionInfo version;

  version.software = SOFTWARE_VERSION;
  version.hardware = HARDWARE_VERSION;
  version.firmware = msg->firmware;

  aicon_d_version_info_pub.publish(version);
}

void msgPub()
{
  diagnostic_msgs::DiagnosticArray aicon_d_diagnostics;

  aicon_d_diagnostics.header.stamp = ros::Time::now();

  aicon_d_diagnostics.status.clear();
  aicon_d_diagnostics.status.push_back(imu_state);
  aicon_d_diagnostics.status.push_back(motor_state);
  aicon_d_diagnostics.status.push_back(LDS_state);
  aicon_d_diagnostics.status.push_back(battery_state);
  aicon_d_diagnostics.status.push_back(button_state);

  aicon_d_diagnostics_pub.publish(aicon_d_diagnostics);
}

int main(int argc, char **argv)
{
  ros::init(argc, argv, "aicon_diagnostic");
  ros::NodeHandle nh;

  aicon_d_diagnostics_pub  = nh.advertise<diagnostic_msgs::DiagnosticArray>("diagnostics", 10);
  aicon_d_version_info_pub = nh.advertise<aicon_msgs::VersionInfo>("version_info", 10);

  ros::Subscriber imu         = nh.subscribe("imu", 10, imuMsgCallback);
  ros::Subscriber lds         = nh.subscribe("scan", 10, LDSMsgCallback);
  ros::Subscriber aicon_d_sensor  = nh.subscribe("sensor_state", 10, sensorStateMsgCallback);
